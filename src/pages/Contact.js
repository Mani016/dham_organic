...
const Contact =()=>{
  ...
  function handleSubmit(){
    ...
    agent.Contact.send(data).then((res) => {
				                            ...
				                            <form method="post"  name="contact-form" id="contact-form">
				                                <div className="row">
				                                    <div className="col-lg-6 col-sm-12"> 
				                                        <input type="text" id="name" value={name} onChange={({target})=>setName(target.value)} className="con-field form-control" placeholder="Name*" /> 
				                                        
				                                    </div>
				                                    <div className="col-lg-6 col-sm-12"> 
				                                        <input type="text" className="con-field form-control" value={mobile} onChange={({target})=>setMobile(target.value)} id="exampleInputEmail1" placeholder="Mobile*" />
				                                            
				                                    </div>                            
				                                    <div className="col-lg-12 col-sm-12"> 
				                                        <input type="text" id="subject" value={address} onChange={({target})=>setAddress(target.value)} className="form-control con-field" placeholder="Your Address*" />
				                                       
				                                    </div>
				                                </div>
				                                <div className="row">
				                                    <div className="col-lg-12 col-sm-12"> 
				                                        <textarea name="comments" id="comments" value={message} onChange={({target})=>setMessage(target.value)} rows="6" className="form-control con-field" placeholder="Your Message*"></textarea>
				                                        <div className="submit-area"> 
				                                            <input type="button" id="submit" name="send" onClick={handleSubmit} className="submit-contact submitBnt" value="Send Message" /> 
